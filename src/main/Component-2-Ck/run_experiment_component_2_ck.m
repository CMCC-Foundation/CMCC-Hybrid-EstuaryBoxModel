...
% Ck coefficient. The aim is to develop two machine 
% learning models (Random Forest, LSBoost).  

%% Add path and directory
addpath(genpath("..\..\lib\config\"));
load_path_hyb_ebm_model(2);

%% Read dataset
ck_dataset_rf = import_dataset("Ck-Obs-RF\Ck-Dataset-RF.xlsx", 6, "A2:F1462", ...
    "Ck_dataset_rf", ["Year","Doy","Qriver", "Qtidef", "Socean", "CkObs"], ...
    ["int16","int16","double", "double","double","double"]);

ck_dataset_lsboost = import_dataset("Ck-Obs-LSBoost\Ck-Dataset-LSBoost.xlsx", 6, "A2:F1462", ...
    "Ck_dataset_lsboost", ["Year","Doy","Qriver", "Qtidef", "Socean", "CkObs"], ...
    ["int16","int16","double", "double","double","double"]);
...
%writetable(ck_training_dataset_rf, '..\..\..\data\input\Component-2-Ck\Ck-Obs-RF\Ck-Training-Dataset-RF.xlsx', 'WriteRowNames',true);
%writetable(ck_test_dataset_rf, '..\..\..\data\input\Component-2-Ck\Ck-Obs-RF\Ck-Test-Dataset-RF.xlsx', 'WriteRowNames',true);
%writetable(ck_training_dataset_lsboost, '..\..\..\data\input\Component-2-Ck\Ck-Obs-LSBoost\Ck-Training-Dataset-LSBoost.xlsx', 'WriteRowNames',true);
%writetable(ck_test_dataset_lsboost, '..\..\..\data\input\Component-2-Ck\Ck-Obs-LSBoost\Ck-Test-Dataset-LSBoost.xlsx', 'WriteRowNames',true);
...
component_2_trained_models.random_forest = random_forest( ...
    ck_training_dataset_rf(:,["Qriver","Qtidef","Socean","CkObs"]), ...
...
component_2_trained_models.random_forest.test_results.test_predictions = ...
    component_2_trained_models.random_forest.model.predictFcn(ck_test_dataset_rf(:,["Qriver","Qtidef","Socean"]));
...
component_2_trained_models.lsboost = lsboost( ...
    ck_training_dataset_lsboost(:,["Qriver","Qtidef","Socean","CkObs"]), ...
    targetFeatureName, max_objective_evaluations, k);
...
component_2_trained_models.lsboost.test_results.test_predictions = ...
    component_2_trained_models.lsboost.model.predictFcn(ck_test_dataset_lsboost(:,["Qriver","Qtidef","Socean"]));
