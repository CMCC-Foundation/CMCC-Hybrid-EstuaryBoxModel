%% Function to train and test a regression tree model
%% Input:
%  1) trainingDataset: 
...
function [results] = regression_tree_function(trainingDataset,targetFeatureName,max_objective_evaluations, k)
...
rng('default');
regression_tree_settings_optimized = fitrtree( ...
    predictors, response, ...
    'Surrogate','on',...
...
ensemblePredictFcn = @(x) predict(regression_tree_settings_optimized, x);
trainedModel.predictFcn = @(x) ensemblePredictFcn(predictorExtractionFcn(x));

%% Add additional fields to the result struct
trainedModel.RequiredVariables = trainingDataset.Properties.VariableNames;
trainedModel.RegressionEnsemble = regression_tree_settings_optimized;
trainedModel.About = 'This struct is a regression tree optimized trained model.';
...
results = struct('model', trainedModel, 'rmse', validationRMSE,...
    'predictions', validationPredictions, 'hyperparameters', regression_tree_settings_optimized);
