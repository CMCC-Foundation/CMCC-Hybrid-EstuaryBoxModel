...
load("..\1-Trained-Models\Trained-model-k-4.mat");

numElDataset = numel(lx_dataset.Lx_OBS);
resumeTable = array2table([lx_dataset.Lx_OBS result_trained_model.random_forest.predictions...
    abs(lx_dataset.Lx_OBS - result_trained_model.random_forest.predictions)],...
...
resumeTable = array2table([lx_dataset.Lx_OBS result_trained_model.lsboost.predictions...
    abs(lx_dataset.Lx_OBS - result_trained_model.lsboost.predictions)],...
    'VariableNames',{'Lx_OBS','Predicted','Residuals'} );
resumeTable = sortrows(resumeTable,{'Lx_OBS'},{'ascend'});
resumeTable.ID = linspace(1,numElDataset,numElDataset)';
subplot(1,2,2);
...
%{
resumeTable = array2table([lx_dataset.Lx_OBS result_trained_model.neural_network.predictions...
    abs(lx_dataset.Lx_OBS - result_trained_model.neural_network.predictions)],...
    'VariableNames',{'Lx_OBS','Predicted','Residuals'} );
resumeTable = sortrows(resumeTable,{'Lx_OBS'},{'ascend'});
resumeTable.ID = linspace(1,numElDataset,numElDataset)';
subplot(1,3,3);
