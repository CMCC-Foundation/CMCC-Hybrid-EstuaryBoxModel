...
load('..\..\0-Dataset\training_2016_2017_test_2018_2019_comparing_old_model\Q_river_Class_splitted\CK_Q_RIVER_CLASS_SPLIT.mat');
load(strcat("..\..\1-Trained-Models\training_2016_2017_test_2018_2019_comparing_old_model\Q_river_Class_splitted\",q_river_selected_class,'\Ck-Trained-Tested-model.mat'));

algorithm_names = {'old model','random forest', 'lsboost', 'neural network'};
response = 'CkObs';

%% Test dataset
test_table_results = array2table([ck_dataset.CkObs((ck_dataset.Year == 2018 | ck_dataset.Year == 2019) & strcmp(string(ck_dataset.QriverClass), q_river_selected_class))...
    ck_dataset.CkOldmodel((ck_dataset.Year == 2018 | ck_dataset.Year == 2019) & strcmp(string(ck_dataset.QriverClass), q_river_selected_class))...
